...
namespace Tesst1.Controllers
{
    public class AdminProductController : Controller
    {
        ...
        {
            var query = db.Products.Include(p => p.Categories);
            ViewBag.KeyWord = searchString;
            if (!string.IsNullOrEmpty(searchString))

                query = query.Where(p => p.ProductName.Contains(searchString) || p.Categories.CategoryName.Contains(searchString));
        ...
        public ActionResult Create([Bind(Include = "ProductID,ProductName,ProductDescription,ProductPrice,ProductImage,ProductQuantity,ProductSold,CategoryID")] Products products, HttpPostedFileBase ProductImage)
        {
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult Edit([Bind(Include = "ProductID,ProductName,ProductDescription,ProductPrice,ProductImage,ProductQuantity,ProductSold,CategoryID")] Products products, HttpPostedFileBase ProductImage, FormCollection form)
        {
            if (ModelState.IsValid)
            {
                try
